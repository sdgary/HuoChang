...
import byxx.android.huochang.gdcarcheck.others.CCSortMenuFragment;
...
//巡岗
public class AcGdCarCheck extends SlidingActivity implements ActionBar.TabListener {

private  String[] CONTENT ;//= new String[] { "101", "102", "103"};
private FragmentManager mFt = getSupportFragmentManager();
private CCSortMenuFragment mMenuFragment;
...
//	AppManager.getAppManager().addActivity(this);
	setContentView(R.layout.carcheck_sliding);
	...
	mMenuFragment = new CCSortMenuFragment();
	fragmentTransaction.replace(R.id.menu, mMenuFragment);
	initSlidingMenu();
	fragmentTransaction.commit();
	int i  = getIntent().getIntExtra("one_num", 0);
...
class MyAdapter extends FragmentPagerAdapter {
	...
	public Fragment getItem(int position) {
		...
		return AcCarCheckFragment.newInstance(CONTENT[position],position);//(CONTENT[position % CONTENT.length]);
	}
...
//		}
    ...
    if(keyCode == KeyEvent.KEYCODE_BACK){    //点击返回按键  
		...
		intent.setClass(AcGdCarCheck.this,AcFunction.class);
